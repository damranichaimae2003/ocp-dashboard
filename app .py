...
col1.metric(
    label="⏱️ Heures de marche",
    value=f"{lr['heures_marche']:,.0f} h",
    delta="Cumulées (SMH)"
)
col2.metric(
    label="🌡️ Temp. refroidissement",
    value=f"{lr['temp_refroid']:.1f} °C",
    delta="⚠️ Seuil : 105 °C" if lr['temp_refroid'] > 95 else "Normal"
)
col3.metric(
    label="⚡ Régime moteur",
...
col4.metric(
    ...
    delta="⚠️ Bas" if lr['niveau_carburant'] < 25 else "OK"
)
col5.metric(
    label="🔴 DTC Actifs",
    value=int(nb_dtc),
...
with chart_col1:
    fig_hm = px.line(
        ...
        labels={"horodatage": "Date/Heure UTC",
...
if dtc_df.empty:
    ...
    st.dataframe(
        dtc_df.style.applymap(highlight_statut, subset=["statut"]),
...
st.markdown("---")
st.markdown(
    """<div style="text-align:center; color:#2E5370; font-size:0.75rem;">
